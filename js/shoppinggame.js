...
const dateDiff = (date1, date2) => {
    ...
    let dateInDays = Math.ceil(dateDiff / (3600 * 1000 * 24));
    return dateInDays;
};

// Here, use Object.defineProperty to create property - daysToExpire
Object.defineProperty(Product.prototype, "daysToExpire", {
    get: function() { return dateDiff(this.daysToExpire, new Date())}
});
...
function init(data) {
    if (Object.is(data, undefined) == false && gameComplete == true) {
        ...
        console.log(
            "You can start the game or quit using the following options.",
        );
        ...
        function (option) {
            ...
            if (regexYes.test(option)) {
                ...
                console.log(
                    `${
                        player.name
                    } you earned ${player.getCurrentScore()} points!`.bold,
                );
                if (player.score >= 500) {
    ...
    rl.question(
        ...
        function (r) {
            if (r == "" || isNaN(r) || r == 0 || r > 10) {
                console.log(
                    "Invalid rating! Please nter a number from 1 - 10".red,
                );
                rateAndExit();
            } else {
                ...
                console.log(
                    `${target.name} you rated this game as ${target.rate}`
                        .green,
                );
